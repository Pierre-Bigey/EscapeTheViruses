...
public class TrailEndanger : MonoBehaviour
{
    ...
    [Tooltip("The probability of having two dangers on the same z value")]
    [SerializeField] private float probabilityOfTwoDangers = 0.6f;
    
    [Header("Dangers Settings")]
    
    [Tooltip("The dangers prefab")]
    [SerializeField] private GameObject dangerPrefab;
    
    [Tooltip("The distance between two dangers")] 
    [SerializeField] private float dangerSpacing = 5f;
    
    [Tooltip("The x space for dangers to spawn on the platform")]
    ...
    private void EndangerPlatform()
    {
        for (float z = 0; z < transform.localScale.z; z += dangerSpacing)
        {
            ...
            Vector3 cubePosition = new Vector3(x, 1f, z+transform.position.z- transform.localScale.z/2);
            ...
            if(Random.value <= probabilityOfTwoDangers)
            {
                ...
                cubePosition = new Vector3(x, 1f, z+transform.position.z- transform.localScale.z/2);
